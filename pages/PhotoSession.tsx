      ...
      <div className="w-[360px] shrink-0 flex flex-col" style={{ background:'var(--bg-1)', borderRight:'1px solid var(--border)' }}>
        ...
        <div className="px-4 py-3" style={{ borderBottom:'1px solid var(--border)' }}>
          <div className="flex items-center justify-between mb-2">
            <div className="text-[9px] font-mono uppercase tracking-wider" style={{ color:'var(--text-3)' }}>Source</div>
            <div className="flex gap-0.5 p-0.5 rounded-lg" style={{ background:'var(--bg-3)' }}>
              {(['gallery', 'upload', 'character'] as const).map(mode => (
                <button key={mode} onClick={() => setSourceMode(mode)}
                  ...
                  style={{
                    background: sourceMode === mode ? 'rgba(240,104,72,.1)' : 'transparent',
                    color: sourceMode === mode ? 'var(--accent)' : 'var(--text-3)',
                  }}>
          ...
          {sourceMode === 'gallery' ? (
            ...
            <div>
              {recentPhotos.length === 0 ? (
                <div className="text-[11px] py-3 text-center" style={{ color:'var(--text-3)' }}>
                  No gallery photos yet. Generate some in Director first.
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-4 gap-1.5 max-h-[140px] overflow-y-auto" style={{ scrollbarWidth:'thin' }}>
                    {recentPhotos.map(g => (
                      <button key={g.id} onClick={() => setSelectedGalleryItem(g)}
                        ...
                        style={{
                          border: `2px solid ${selectedGalleryItem?.id === g.id ? 'var(--accent)' : 'var(--border)'}`,
                        }}>
                  ...
                  {selectedGalleryItem && (
                    <div className="flex items-center gap-2 mt-2">
                      <img src={selectedGalleryItem.url} className="w-10 h-10 rounded-xl object-cover" alt="" />
                      <div className="flex-1 min-w-0">
                        <div className="text-[11px] font-medium truncate" style={{ color:'var(--text-1)' }}>Base photo selected</div>
                        <div className="text-[9px] truncate" style={{ color:'var(--text-3)' }}>
                      ...
                      <button onClick={() => setSelectedGalleryItem(null)}
                        ...
                        style={{ background:'var(--bg-3)', color:'var(--text-3)' }}>{'\u2715'}</button>
                    </div>
                  )}
                </>
              )}
              <div className="flex items-center gap-2 mt-2">
                ...
                <button onClick={() => setShowEngineModal(!showEngineModal)}
                  className="w-9 h-9 rounded-xl flex items-center justify-center transition-all hover:scale-105 shrink-0"
                  style={{
            ...
            <div className="flex gap-2 items-center">
              {uploadedSubject ? (
                <div className="flex items-center gap-2 flex-1">
                  <img src={uploadedSubject.preview} className="w-10 h-10 rounded-xl object-cover" alt="" />
                  <div className="flex-1 min-w-0">
                    <div className="text-[11px] font-medium truncate" style={{ color:'var(--text-1)' }}>Photo uploaded</div>
                    <button onClick={() => subjectInputRef.current?.click()}
                      className="text-[9px]" style={{ color:'var(--accent)' }}>Change</button>
                  </div>
                ...
                <button onClick={() => subjectInputRef.current?.click()}
                  className="flex-1 py-3 rounded-xl text-center transition-all hover:border-[var(--border-h)]"
                  style={{ background:'var(--bg-3)', border:'1px dashed var(--border)' }}>
                  <span className="text-sm block mb-0.5" style={{ color:'var(--accent)' }}>{'\u2191'}</span>
                  <span className="text-[10px]" style={{ color:'var(--text-2)' }}>Upload a photo</span>
                </button>
              )}
              {/* Engine selector */}
              <button onClick={() => setShowEngineModal(!showEngineModal)}
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
          ...
          <div>
            ...
            <div className="text-[8px] mb-2" style={{ color:'var(--text-3)', opacity: 0.6 }}>
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-1.5" style={{ scrollbarWidth:'none' }}>
              {scenes.map(s => (
                <button key={s.id} onClick={() => { setSelectedScene(selectedScene === s.id ? '' : s.id); setCustomScene('') }}
                  ...
                  style={{
                    background: selectedScene === s.id && !customScene ? 'rgba(240,104,72,.08)' : 'var(--bg-3)',
                    border: `1px solid ${selectedScene === s.id && !customScene ? 'rgba(240,104,72,.2)' : 'var(--border)'}`,
                    color: selectedScene === s.id && !customScene ? 'var(--accent)' : 'var(--text-2)',
                  }}>
            ...
            <input type="text" placeholder="Or describe a custom scene override..."
              ...
              style={{
                background: 'var(--bg-3)',
                borderColor: customScene ? 'rgba(240,104,72,.2)' : 'var(--border)',
                color: 'var(--text-1)',
              }}
              value={customScene}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <div className="text-[9px] font-mono uppercase tracking-wider" style={{ color:'var(--text-3)' }}>Pick your vibes</div>
              ...
              {PHOTO_SESSION_PRESETS.map(p => {
                ...
                return (
                  <button key={p.id} onClick={() => togglePreset(p.id)}
                    ...
                    style={{
                      background: active ? 'rgba(240,104,72,.08)' : 'var(--bg-3)',
                      border: `1px solid ${active ? 'rgba(240,104,72,.25)' : 'var(--border)'}`,
                    }}>
                    ...
                    <div className="text-[11px] font-medium" style={{ color: active ? 'var(--accent)' : 'var(--text-2)' }}>{p.label}</div>
                    <div className="text-[8px] mt-0.5 line-clamp-1" style={{ color:'var(--text-3)' }}>{p.description}</div>
                  </button>
                )
              })}
            </div>
          ...
          <div>
            <div className="text-[9px] font-mono uppercase tracking-wider mb-2" style={{ color:'var(--text-3)' }}>
              Reference <span style={{ color:'var(--text-3)', opacity: 0.5 }}>(optional)</span>
            </div>
            ...
            {refImage ? (
              <div className="relative aspect-video rounded-xl overflow-hidden"
                ...
                <div className="absolute bottom-1.5 left-1.5 px-2 py-0.5 rounded-md text-[9px] backdrop-blur-sm"
                  style={{ background:'rgba(0,0,0,.5)', color:'var(--text-1)' }}>Look reference</div>
              </div>
            ) : (
              <div className="flex gap-2">
                <button onClick={() => refInputRef.current?.click()}
                  ...
                  style={{ background:'var(--bg-3)', border:'1px dashed var(--border)' }}>
                  <span className="text-sm block mb-0.5" style={{ color:'var(--accent)' }}>{'\u2191'}</span>
                  <span className="text-[10px]" style={{ color:'var(--text-2)' }}>Upload</span>
                </button>
        ...
        <div className="px-4 py-3 shrink-0 space-y-3" style={{ borderTop:'1px solid var(--border)' }}>
          <div>
            <div className="text-[9px] font-mono uppercase tracking-wider mb-2" style={{ color:'var(--text-3)' }}>Photos</div>
            ...
            <div className="mt-1.5 flex items-center justify-between">
              <div className="text-[9px] truncate pr-2" style={{ color:'var(--text-3)' }}>
              ...
              <div className="absolute inset-0">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-[11px]" style={{ color:'var(--text-2)' }}>Base photo ready — pick vibes and shoot</p>
                  </div>
                </div>
              </div>
            ) : (
            ...
            <div className="absolute top-3 left-3 right-3 flex flex-wrap gap-1.5">
              ...
              {PHOTO_SESSION_PRESETS.filter(p => selectedPresets.has(p.id)).map(p => (
                <span key={p.id} className="px-2 py-0.5 rounded-md text-[9px] font-mono backdrop-blur-sm"
                  style={{ background:'rgba(0,0,0,.4)', color:'var(--accent)' }}>
                  {p.icon} {p.label}
                </span>
              ))}
            </div>
            ...
            {generatedImages.length === 0 && !baseImagePreview && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <p className="text-[11px]" style={{ color:'var(--text-3)' }}>Pick a base photo and vibes, then shoot</p>
                </div>
              </div>
            )}
          </div>
