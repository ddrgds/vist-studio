...
export function StudioV2({ onNav, onEditImage, onExportImage }: {
  ...
  const handleHeroGenerate = async () => {
    ...
    // Premium preprocessing flags — outer scope so the catch handler can reference them.
    ...
    try {
      ...
      if (poseRef && posePrecise) {
        ...
        if (okPose) {
          ...
          try {
            ...
            const c = await runControlNet(u, params.scenario || '')
            if (c) {
      ...
      if (eng?.provider === AIProvider.Higgsfield) results = await generateWithSoul(params, p => setHeroProgress(p), abortHeroRef.current.signal)
        ...
        try {
          ...
          console.warn('NB2 hero failed, trying Grok:', nb2Err)
