// Cloudflare Pages Function — Higgsfield API proxy
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  const targetUrl = `https://platform.higgsfield.ai/${pathStr}${url.search}`;

  const headers = new Headers(request.headers);
  headers.set('Authorization', `Key ${env.HIGGSFIELD_API_KEY}:${env.HIGGSFIELD_API_SECRET}`);
