    ...
    if (aistudio && aistudio.openSelectKey) {
      try {
        ...
        if (error.message && error.message.includes("Requested entity was not found")) {
            setHasKey(false);
            setErrorMessage("No se pudo seleccionar el proyecto. Por favor, intenta de nuevo.");
        } else {
            setErrorMessage("Error al seleccionar la API key. Por favor, intenta de nuevo.");
        }
      }
    } else {
        setErrorMessage("La interfaz de selección de API key no está disponible en este entorno.");
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 text-zinc-400" aria-live="polite" aria-busy="true">
        <div className="flex flex-col items-center gap-3">
            ...
            <span className="text-sm">Iniciando Estudio...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-950 p-4">
        <div className="max-w-md w-full bg-zinc-900 border border-zinc-800 rounded-2xl p-8 text-center shadow-2xl">
          ...
          <h1 className="text-2xl font-bold text-white mb-3">Autenticación requerida</h1>
          <p className="text-zinc-400 mb-8">
            Para usar las funciones de generación de imágenes (Gemini 3 Pro), debes seleccionar una API key válida con facturación habilitada.
          </p>
