  ...
  if (presets.length === 0) {
    return (
      <div className="space-y-4 pt-6 border-t border-zinc-800">
        <h2 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider flex items-center gap-2">My Presets</h2>
        <p className="text-xs text-zinc-600 text-center py-2">
          Save your first preset with the button above.
        </p>
      </div>
    );
  }
        ...
        {presets.map(preset => (
          ...
          >
            {confirmDeleteId === preset.id ? (
              ...
              >
                <span className="text-[10px] text-zinc-300 flex-1 truncate">Delete "{preset.name}"?</span>
                <button
              ...
              <>
                ...
                >
                  {/* Thumbnail — preset reference photo */}
                  {preset.thumbnail ? (
                    <img
                      src={preset.thumbnail}
                      alt={`Thumbnail of ${preset.name}`}
                  ...
                  <div className="min-w-0">
                    ...
                    {/* Small summary of what the preset contains */}
