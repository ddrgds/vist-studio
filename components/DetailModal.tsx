  ...
  const renderParams = () => {
    if (item.type === 'create') {
        ...
        return (
            <div className="space-y-4">
                <div className="bg-zinc-800/50 p-3 rounded-lg border border-zinc-700">
                    <span className="text-xs text-zinc-500 uppercase tracking-wider font-bold">Concept</span>
                    {characteristics && <div className="mt-1"><span className="text-zinc-400 text-xs">Characteristics:</span> <p className="text-sm text-zinc-200">{characteristics}</p></div>}
                    ...
                    <div className="mt-2">
                        ...
                        {!pose && !poseUrl && <p className="text-sm text-zinc-500 italic">Not specified</p>}
                    </div>

                    <div className="mt-2">
                        <span className="text-zinc-400 text-xs">Scenario:</span>
                        ...
                        {scenarioUrl && (
                            <div className="mt-1 h-16 w-24 rounded border border-zinc-700 overflow-hidden relative group">
                                <img src={scenarioUrl} className="w-full h-full object-cover" alt="Scenario reference image" />
                                <div aria-hidden="true" className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 text-[8px] text-white">Ref Scenario</div>
                            </div>
                        )}
                        {!p.scenario && !scenarioUrl && <p className="text-sm text-zinc-500 italic">Not specified</p>}
                    </div>

                    <div className="mt-2">
                        <span className="text-zinc-400 text-xs">Object / Accessory:</span>
                        {accessory && <p className="text-sm text-zinc-200">{accessory}</p>}
                        {accessoryUrl && (
                            <div className="mt-1 h-16 w-16 rounded border border-zinc-700 overflow-hidden relative group">
                                <img src={accessoryUrl} className="w-full h-full object-cover" alt="Accessory reference image" />
                                <div aria-hidden="true" className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 text-[8px] text-white">Ref Object</div>
                            </div>
                        )}
                        {!accessory && !accessoryUrl && <p className="text-sm text-zinc-500 italic">None</p>}
                    </div>

                    {p.lighting && <div className="mt-2"><span className="text-zinc-400 text-xs">Lighting:</span> <p className="text-sm text-zinc-200">{p.lighting}</p></div>}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                     ...
                     {p.cfg && (
                         <div className="bg-zinc-800/30 p-2 rounded border border-zinc-800">
                            <span className="text-[10px] text-zinc-500 block">Guidance (CFG)</span>
                     ...
                     <div className="bg-zinc-800/30 p-2 rounded border border-zinc-800 col-span-2">
                        <span className="text-[10px] text-zinc-500 block">Main References</span>
        ...
        return (
             <div className="space-y-4">
                 <div className="bg-zinc-800/50 p-3 rounded-lg border border-zinc-700">
                    <span className="text-xs text-zinc-500 uppercase tracking-wider font-bold">Script</span>
                    <div className="mt-1"><span className="text-zinc-400 text-xs">Action Prompt:</span> <p className="text-sm text-zinc-200">{p.prompt}</p></div>
                    {p.dialogue && <div className="mt-2"><span className="text-zinc-400 text-xs">Dialogue:</span> <p className="text-sm text-zinc-200 italic">"{p.dialogue}"</p></div>}
                </div>
                <div className="grid grid-cols-2 gap-3">
                     <div className="bg-zinc-800/30 p-2 rounded border border-zinc-800">
                        <span className="text-[10px] text-zinc-500 block">Resolution</span>
    ...
    >
      ...
      >
        ...
        <div className="flex-1 bg-black flex items-center justify-center p-8 relative group h-[45vh] lg:h-auto border-b lg:border-b-0 border-zinc-800">
             {item.type === 'video' ? (
                <video controls src={item.url} aria-label="Generated video" className="max-w-full max-h-full object-contain rounded-lg shadow-2xl" />
            ) : (
                <img src={item.url} alt="Generated image" className="max-w-full max-h-full object-contain rounded-lg shadow-2xl" />
            )}
        ...
        <div className="w-full lg:w-96 border-l border-zinc-800 bg-zinc-900 flex flex-col shrink-0 h-[55vh] lg:h-auto">
            <div className="p-6 border-b border-zinc-800">
                ...
                <h2 id="detail-modal-title" className="text-xl font-bold text-white">Generation Details</h2>
            </div>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar flex-1">
                ...
                <div className="mt-8 pt-6 border-t border-zinc-800">
                    <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Resource ID</h3>
